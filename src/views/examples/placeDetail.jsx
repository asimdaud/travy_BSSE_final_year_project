...
class PlaceDetail extends Component {
  getPlaceDetails = () => {
    ...
    let url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place_id}&fields=name,geometry,photos,rating,formatted_phone_number&key=${process.env.REACT_APP_GOOGLE_API_KEY}`;
    console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((result) => {
        ...
        res.photos.forEach((photo) => {
          let url = `https://maps.googleapis.com/maps/api/place/photo?photoreference=${photo.photo_reference}&sensor=false&maxheight=${photo.height}&maxwidth=${photo.width}&key=${process.env.REACT_APP_GOOGLE_API_KEY}`;
          photos.push(url);
        });
        let location = {
